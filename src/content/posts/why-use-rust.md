...
My favourite thing with Rust by far is the type system. It's easy to grok and really powerful and convienent to use.
...
Your old projects will still compile with newer versions of the compilier. No worring about enviornment management just use what is up todate.

### Single Binary

No enviornment variables to deal with, just copy one file and it'll just work.
...
It's not just one person or company that's maintaining it. It's a community effort. I think this is a huge plus, it make the language take more time to make decisions, but it makes the language well thought out.

One of thing that was debated about a lot was the was how to implement the async await sytnax. In the end the postfix syntax was chosen. I was just writing some ECMAScript and missed my postfixed `await`. It makes chaining functions together so much easier.

Another benifit from having a community is they choose their favourite parts of other languages and bring them to Rust.
